...
export function AdminRelationshipTypes() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['admin-relationship-types', { search }],
    queryFn: () => adminApi.relationshipTypes.list(),
  })

  const createMutation = useMutation({
    mutationFn: (data: any) => adminApi.relationshipTypes.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-relationship-types'] })
      setShowCreateModal(false)
    },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => adminApi.relationshipTypes.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-relationship-types'] })
      setEditingItem(null)
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => adminApi.relationshipTypes.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-relationship-types'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-secondary-900">Tipos de Relacionamento</h1>
          <p className="text-secondary-500">Gerenciar tipos de conexões entre ativos</p>
        </div>
              ...
              {isLoading ? (
                ...
                data?.map((item) => (
                  <tr key={item.id} className="hover:bg-secondary-50">
                    ...
                    <td className="px-4 py-3 text-sm text-secondary-600 max-w-md truncate">{item.descricao || '—'}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          onClick={() => { if (confirm('Excluir este tipo?')) deleteMutation.mutate(item.id) }}
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6 border-b border-secondary-200">
              <h2 className="text-xl font-bold text-secondary-900">{editingItem ? 'Editar Tipo' : 'Novo Tipo de Relacionamento'}</h2>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); const formData = new FormData(e.currentTarget); const data = Object.fromEntries(formData); if (editingItem) updateMutation.mutate({ id: editingItem.id, data }); else createMutation.mutate(data) }}>
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input name="nome" type="text" className="input" defaultValue={editingItem?.nome || ''} required />
                </div>
                <div>
                  <label className="label">Descrição</label>
                  <textarea name="descricao" className="input" rows={3} defaultValue={editingItem?.descricao || ''} />
                </div>
              </div>
