...
export function AdminAssetTypes() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['admin-asset-types', { search }],
    queryFn: () => adminApi.assetTypes.list(),
  })

  const createMutation = useMutation({
    mutationFn: (data: any) => adminApi.assetTypes.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => adminApi.assetTypes.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-asset-types'] })
      setEditingItem(null)
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => adminApi.assetTypes.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-asset-types'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-secondary-900">Tipos de Ativo</h1>
          <p className="text-secondary-500">Gerenciar categorias de ativos de infraestrutura</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6 border-b border-secondary-200">
              <h2 className="text-xl font-bold text-secondary-900">{editingItem ? 'Editar Tipo de Ativo' : 'Novo Tipo de Ativo'}</h2>
            </div>
